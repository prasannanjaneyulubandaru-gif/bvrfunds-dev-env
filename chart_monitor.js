...
async function stopMonitor() {
    if (!monitorState.isRunning) { addLog('Monitor is not running', 'info'); return; }
    try {
        const userId = sessionStorage.getItem('user_id');
...
async function checkNow() {
    ...
    const interval = document.getElementById('intervalSelect').value;
    const threshold = document.getElementById('thresholdPercent').value;
    if (!instrumentToken || !interval || !threshold) { addLog('Please fill all fields', 'error'); return; }
...
function startStatusPolling() {
    ...
    monitorState.intervalId = setInterval(async () => {
        try {
            const userId = sessionStorage.getItem('user_id');
            if (!userId) { stopStatusPolling(); return; }
            const response = await fetch(`${CHART_CONFIG.backendUrl}/api/monitor-status`, {
                headers: { 'X-User-ID': userId }
            });
            if (response.status === 401) {
...
function updateMonitorUI(isRunning) {
    ...
    const startBtn = document.getElementById('startMonitorBtn');
    const stopBtn = document.getElementById('stopMonitorBtn');
    if (!statusDiv || !startBtn || !stopBtn) return;
    if (isRunning) {
        statusDiv.classList.remove('inactive'); statusDiv.classList.add('active');
        ...
        statusDiv.classList.remove('active'); statusDiv.classList.add('inactive');
